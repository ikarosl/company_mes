-- Keep one material-demand row per production batch and BOM item.
-- Existing active rows are preferred; otherwise the newest historical row is kept.
SET NAMES utf8mb4;
USE `company_test`;

DELETE duplicate_row
FROM batch_material_usages duplicate_row
INNER JOIN (
  SELECT
    grouped.batch_id,
...
ALTER TABLE batch_material_usages
  ADD UNIQUE KEY `uk_batch_material_usages_batch_material` (`batch_id`, `product_materials_id`);
