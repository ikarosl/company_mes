-- Scheme 2 database baseline.
-- Unchanged modules retain the current company_test structure.
-- Legacy material_batches and batch_material_usages are intentionally excluded.
-- Warehouse material tables follow docs/newSqlDesign.md and contain no seed data.

CREATE DATABASE IF NOT EXISTS `company_mes_v2`
...
CREATE TABLE `material_batch` (
  ...
  `batch_status` varchar(20) DEFAULT '可用'
    COMMENT '批次业务状态：可用/冻结/停用。不要用它表示用完，用完应由库存流水汇总判断',
...
CREATE TABLE `inventory_transaction` (
  ...
  `transaction_type` varchar(30) NOT NULL
    COMMENT '库存变动类型：入库/出库/退料入库/报废出库/盘点调整/状态转入/状态转出',
  `quantity` decimal(12,4) NOT NULL COMMENT '库存变动数量。正数表示增加，负数表示减少',
  `stock_status` varchar(20) DEFAULT '可用'
    COMMENT '库存状态：可用/待检/冻结/不良。可分配库存只统计 stock_status=可用',
  `reference_type` varchar(50) DEFAULT NULL
    COMMENT '来源明细类型：PO_DETAIL/OUTBOUND_DETAIL/RETURN_DETAIL/SCRAP/STOCK_CHECK/STATUS_TRANSFER',
  ...
  CONSTRAINT `fk_inventory_transaction_batch_material`
    FOREIGN KEY (`batch_id`, `material_id`) REFERENCES `material_batch` (`id`, `material_id`),
...
CREATE TABLE `material_demand` (
  ...
  `status` varchar(30) DEFAULT '待分配'
    COMMENT '状态：待分配/部分分配/已分配/部分出库/已出库/退料处理中/报废处理中/已关闭/已取消',
  `allocated_quantity` decimal(12,4) DEFAULT 0
    COMMENT '累计已分配数量，来自 material_allocation.assigned_number 汇总',
  `outbound_quantity` decimal(12,4) DEFAULT 0
    COMMENT '累计出库数量，来自 outbound_detail.outbound_number 汇总',
  `returned_quantity` decimal(12,4) DEFAULT 0
    COMMENT '累计退料数量，来自 return_detail.return_number 汇总',
  `scrapped_quantity` decimal(12,4) DEFAULT 0
    COMMENT '累计报废数量，来自 material_scrap.scrap_number 汇总',
  ...
  CONSTRAINT `fk_material_demand_parent`
    FOREIGN KEY (`parent_demand_id`) REFERENCES `material_demand` (`id`),
...
CREATE TABLE `material_allocation` (
  ...
  `outbound_quantity` decimal(12,4) DEFAULT 0 COMMENT '本分配行累计出库数量',
  `returned_quantity` decimal(12,4) DEFAULT 0 COMMENT '本分配行累计退料数量',
  `scrapped_quantity` decimal(12,4) DEFAULT 0
    COMMENT '本分配行累计报废数量。包括已分配未出库报废，以及退料后报废',
  `version` int DEFAULT 0 COMMENT '乐观锁版本号',
  `created_at` timestamp DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_material_allocation_demand`
    FOREIGN KEY (`demand_id`) REFERENCES `material_demand` (`id`),
  CONSTRAINT `fk_material_allocation_batch`
    FOREIGN KEY (`material_batch_id`) REFERENCES `material_batch` (`id`),
  ...
  CONSTRAINT `chk_material_allocation_available_quantity`
    CHECK (`outbound_quantity` + `scrapped_quantity` <= `assigned_number` + `returned_quantity`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `outbound_order` (
  ...
  CONSTRAINT `fk_outbound_order_demand`
    FOREIGN KEY (`demand_id`) REFERENCES `material_demand` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `outbound_detail` (
  ...
  CONSTRAINT `fk_outbound_detail_order`
    FOREIGN KEY (`outbound_id`) REFERENCES `outbound_order` (`id`),
  CONSTRAINT `fk_outbound_detail_allocation`
    FOREIGN KEY (`allocation_id`) REFERENCES `material_allocation` (`id`),
  CONSTRAINT `chk_outbound_detail_number` CHECK (`outbound_number` > 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- RBAC seed data from docs/init_rbac.sql.
-- Operation logs and refresh tokens are intentionally excluded.
