  ...
  <div class="page">
    ...
    <section class="panel">
      ...
      <el-table v-loading="loading" :data="rows">
        ...
        <el-table-column label="操作" fixed="right" width="210"><template #default="{ row }"><el-button link type="primary" @click="showDetail(row)">详情</el-button><el-button link type="primary" @click="openEdit(row)">编辑</el-button><el-button v-if="hasFailedQuantity(row)" link type="danger" :disabled="row.reworkCount > 0" @click="openRework(row)">{{ row.reworkCount > 0 ? '已创建返工' : '创建返工' }}</el-button></template></el-table-column>
      </el-table>
    ...
    <el-dialog v-model="reworkVisible" title="创建返工单" :width="DialogWidth.lg">
      ...
      <el-form label-width="108px" :model="reworkForm" class="dialog-form"><el-form-item label="返工数量" required><el-input-number v-model="reworkForm.reworkQuantity" :min="0.0001" :precision="4" /></el-form-item><el-form-item label="不合格项" required><el-input v-model="reworkForm.defectItem" /></el-form-item><el-form-item label="问题描述"><el-input v-model="reworkForm.defectDesc" type="textarea" :rows="3" /></el-form-item><el-form-item label="返工工序"><el-input v-model="reworkForm.returnStepName" /></el-form-item><el-form-item label="产品标识"><el-input v-model="reworkForm.productIdentifier" /></el-form-item><el-form-item label="备注"><el-input v-model="reworkForm.remark" type="textarea" :rows="2" /></el-form-item></el-form>
    ...
    <el-dialog v-model="detailVisible" title="检验详情" :width="DialogWidth.xl">
      <el-descriptions v-if="active" :column="2" border>
        <el-descriptions-item label="检验单号">{{ active.inspectionNo || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检验名称">{{ active.inspectionName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检验类型">{{ typeLabel(active.inspectionType) }}</el-descriptions-item>
        <el-descriptions-item label="检验人员">{{ active.inspectorName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="生产批次">{{ active.productionBatchNo || '-' }}</el-descriptions-item>
        <el-descriptions-item label="工单号">{{ active.workOrderNo || '-' }}</el-descriptions-item>
        <el-descriptions-item v-if="active.materialBatchNo" label="物料批次">{{ active.materialBatchNo }}</el-descriptions-item>
        <el-descriptions-item v-if="active.productInventoryId" label="产品库存 ID">{{ active.productInventoryId }}</el-descriptions-item>
        <el-descriptions-item v-if="active.relatedInspectionId" label="来源检验记录 ID">{{ active.relatedInspectionId }}</el-descriptions-item>
        <el-descriptions-item label="产品型号">{{ active.productModel || '-' }}</el-descriptions-item>
        <el-descriptions-item label="产品名称">{{ active.productName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检验工序">{{ active.stepOrder ? `${active.stepOrder}. ` : '' }}{{ active.stepName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检验时间">{{ time(active.inspectedAt) }}</el-descriptions-item>
        <el-descriptions-item label="检验数量">{{ num(active.inspectQuantity) }}</el-descriptions-item>
        <el-descriptions-item label="合格数量">{{ num(active.passQuantity) }}</el-descriptions-item>
        <el-descriptions-item label="不合格数量">{{ num(active.failQuantity) }}</el-descriptions-item>
        <el-descriptions-item label="检验结果"><el-tag :type="resultMeta(active.result).type">{{ resultMeta(active.result).label }}</el-tag></el-descriptions-item>
        <el-descriptions-item label="处置方式">{{ dispositionLabel(active.disposition) }}</el-descriptions-item>
        <el-descriptions-item label="返工单数">{{ active.reworkCount }}</el-descriptions-item>
        <el-descriptions-item label="检测文件" :span="2">
          <el-link v-if="active.fileUrl" type="primary" :href="active.fileUrl" target="_blank">查看检测文件</el-link>
          <span v-else>-</span>
        </el-descriptions-item>
        <el-descriptions-item label="结果说明" :span="2"><div class="detail-text">{{ active.resultSummary || '-' }}</div></el-descriptions-item>
        <el-descriptions-item label="备注" :span="2"><div class="detail-text">{{ active.remark || '-' }}</div></el-descriptions-item>
        <el-descriptions-item label="创建时间">{{ time(active.createdAt) }}</el-descriptions-item>
        <el-descriptions-item label="更新时间">{{ time(active.updatedAt) }}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>
  </div>
</template>
...
const form=reactive(blank()),reworkForm=reactive({reworkQuantity:0,defectItem:'',defectDesc:'',returnStepName:'',productIdentifier:'',remark:''});
...
/** 上传检验附件并保存后端返回的本地访问地址。 */
async function uploadInspectionFile(options:UploadRequestOptions){uploading.value=true;try{const data=new FormData();data.append('file',options.file);const uploaded=await qualityApi.uploadInspectionFile(data);form.fileUrl=uploaded.fileUrl;options.onSuccess(uploaded);EMessage.success('检测文件上传成功')}catch(error){EMessage.error(error,'检测文件上传失败')}finally{uploading.value=false}}
...
async function save(){if(!form.fileUrl)return EMessage.warning('请先上传检测文件');const message=validate();if(message)return EMessage.warning(message);saving.value=true;try{editId.value?await qualityApi.update(editId.value,form):await qualityApi.create(form);EMessage.success('检验记录已保存');formVisible.value=false;await load()}catch(error){EMessage.error(error,'检验记录保存失败')}finally{saving.value=false}}
/** 只有存在不合格数量的检验记录才进入返工判断；是否已创建由 reworkCount 控制。 */
const hasFailedQuantity=(row:InspectionListItem)=>row.result!=='pass'&&Number(row.failQuantity||0)>0;
function openRework(row:InspectionListItem){if(row.reworkCount>0)return EMessage.warning('该检验记录已经创建返工单，请勿重复创建');reworkSource.value=row;Object.assign(reworkForm,{reworkQuantity:Number(row.failQuantity||0),defectItem:row.resultSummary||'',defectDesc:row.resultSummary||'',returnStepName:row.stepName||'',productIdentifier:'',remark:''});reworkVisible.value=true}
