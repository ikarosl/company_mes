...
SELECT
  ...
  requirement.product_materials_id AS product_material_id,
  COALESCE(requirement.material_product_id, pm.material_product_id) AS material_product_id,
  ...
  COALESCE(summary.issued_quantity, 0) - COALESCE(summary.returned_quantity, 0) AS net_used_quantity,
  ...
  COALESCE(summary.allocated_material_batch_count, 0) AS allocated_material_batch_count,
  summary.allocated_material_batch_nos,
  CASE
    WHEN COALESCE(summary.reserved_quantity, 0) < requirement.plan_quantity
      THEN 'shortage'
    WHEN COALESCE(summary.issued_quantity, 0) - COALESCE(summary.returned_quantity, 0) >= requirement.plan_quantity
      THEN 'issued'
    WHEN COALESCE(summary.issued_quantity, 0) - COALESCE(summary.returned_quantity, 0) > 0
      THEN 'partial_issued'
    WHEN COALESCE(summary.reserved_quantity, 0) >= requirement.plan_quantity
      THEN 'allocated'
    WHEN COALESCE(summary.reserved_quantity, 0) > 0
      THEN 'partial_allocated'
    ELSE 'unallocated'
...
FROM batch_material_requirement requirement
INNER JOIN v_production_batch_overview overview
  ON overview.batch_id = requirement.batch_id
INNER JOIN product_materials pm
  ON pm.id = requirement.product_materials_id
  AND pm.is_deleted = 0
INNER JOIN products material
  ON material.id = COALESCE(requirement.material_product_id, pm.material_product_id)
  AND material.is_deleted = 0
LEFT JOIN (
  SELECT
    operation.batch_id,
    operation.require_id,
    ...
    COUNT(DISTINCT CASE WHEN operation.operation_type = 'reserve' THEN operation.material_batch_id END)
      AS allocated_material_batch_count,
    GROUP_CONCAT(
    ...
    SUM(CASE
      ...
      WHEN operation.operation_type = 'unreserve'
        THEN -COALESCE(NULLIF(operation.operation_quantity, 0), operation.reserved_quantity, 0)
      ELSE 0
    ...
    SUM(CASE
      WHEN operation.operation_type = 'return'
        THEN COALESCE(NULLIF(operation.operation_quantity, 0), operation.used_quantity, 0)
      ELSE 0
  ...
  GROUP BY operation.batch_id, operation.require_id, operation.product_materials_id
) summary
  ON summary.batch_id = requirement.batch_id
  AND (
    summary.require_id = requirement.id
    OR (summary.require_id IS NULL AND summary.product_materials_id = requirement.product_materials_id)
  )
