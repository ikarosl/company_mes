...
CREATE TABLE `material_batches` (
  ...
  `quality_status` varchar(50) NOT NULL COMMENT 'qualified/partial_qualified；只有通过来料检验的数量才能入库',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_material_batches_no` (`material_batch_no`),
  ...
  KEY `idx_material_batches_quality_status` (`quality_status`),
  ...
  CONSTRAINT `chk_material_batches_quality_status` CHECK (`quality_status` in ('qualified','partial_qualified'))
